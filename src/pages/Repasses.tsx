...
// Bibliotecas de Exportação (Certifique-se de ter rodado npm install jspdf jspdf-autotable)
...
export function Repasses() {
  ...
  const atendimentosDetalhado = atendimentos.filter(at => {
    ...
    const matchesProf = profissionalSelecionado === "todos" || at.profissional_nome === profissionalSelecionado;
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-8 animate-in fade-in duration-700 text-left">
      
      {/* HEADER E FILTROS */}
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div className="flex items-center gap-4">
          <Button variant="outline" size="sm" onClick={() => navigate('/sistema')} className="rounded-full border-gray-200 hover:bg-gray-100">
        ...
        <div className="flex flex-wrap gap-2">
          <div className="flex bg-gray-100 p-1 rounded-lg mr-2 border border-gray-200">
            <Button onClick={exportarPDF} variant="ghost" size="sm" className="text-red-600 hover:bg-white font-bold gap-1"><FileText size={16} /> PDF</Button>
            <Button onClick={exportarCSV} variant="ghost" size="sm" className="text-emerald-600 hover:bg-white font-bold gap-1"><FileSpreadsheet size={16} /> EXCEL</Button>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card className="bg-blue-600 text-white shadow-xl border-none relative overflow-hidden">
                  ...
                  {relatorio.map(p => (
                    <tr key={p.id} className={`hover:bg-gray-50 transition-colors ${profissionalSelecionado === p.nome ? 'bg-blue-50' : ''}`}>
                      <td className="px-6 py-4 uppercase text-[#1e3a8a] text-xs">
                        {p.nome}
                        <span className="block text-[9px] text-gray-400 font-normal">{p.cargo}</span>
                      </td>
              ...
              atendimentosDetalhado.map((at) => (
                <div key={at.id} className="bg-white border border-gray-100 p-4 rounded-xl shadow-sm hover:border-blue-200 transition-all border-l-4 border-l-blue-500">
                  <div className="flex justify-between items-start mb-1">
                    ...
                    <span className="text-emerald-600 font-black text-xs">R$ {at.valor_atendimento?.toLocaleString('pt-BR')}</span>
                  </div>
                  <h4 className="text-[#1e3a8a] font-bold uppercase text-[11px] truncate">{at.paciente_nome}</h4>
                  <p className="text-[9px] text-gray-400 uppercase font-medium">{at.profissional_nome}</p>
                </div>
              ))
            ) : (
