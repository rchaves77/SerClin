...
export function Relatorios() {
  ...
  const [autorizado, setAutorizado] = useState(false); // NOVO: Controle de Acesso
  ...
  if (!autorizado) return null; // Prevenção de flash de conteúdo
    ...
    <div className="min-h-screen bg-gray-50 p-4 md:p-8 font-sans text-left">
      ...
      <header className="md:hidden bg-white border-b px-4 pb-4 pt-[calc(env(safe-area-inset-top,0px)+12px)] -mx-4 -mt-4 mb-6 sticky top-0 z-40 shadow-sm flex items-center justify-between">
        <div className="flex items-center gap-3">
          <button onClick={() => navigate("/sistema")} className="p-2 -ml-2 text-gray-400"><ArrowLeft size={24} /></button>
          <div>
        ...
        <Button onClick={handleExportarPDF} size="icon" className="bg-blue-600 text-white rounded-xl shadow-md h-10 w-10"><Download size={18} /></Button>
      </header>
      ...
      <div className="max-w-6xl mx-auto space-y-8">
        ...
        <div className="hidden md:flex justify-between items-center print:hidden">
          <Button variant="ghost" onClick={() => navigate("/sistema")} className="gap-2 pl-0 hover:bg-transparent hover:text-blue-600 text-gray-600 font-bold uppercase tracking-widest text-xs">
            <ArrowLeft size={18} /> Voltar ao Painel
          </Button>
          <Button onClick={handleExportarPDF} className="gap-2 bg-blue-600 text-white hover:bg-blue-700 font-black shadow-lg rounded-full px-6 h-12 uppercase tracking-widest text-xs transition-all">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-6 md:p-8 rounded-[2rem] shadow-sm border border-gray-100 relative overflow-hidden">
          ...
          <div className="flex gap-3 items-center print:hidden w-full md:w-auto">
            <select className={`${inputClass} flex-1 md:flex-none h-12 rounded-xl bg-gray-50 border-none uppercase tracking-widest text-xs`} value={mesSelecionado} onChange={(e) => setMesSelecionado(Number(e.target.value))}>
            ...
            <select className={`${inputClass} w-28 h-12 rounded-xl bg-gray-50 border-none uppercase tracking-widest text-xs`} value={anoSelecionado} onChange={(e) => setAnoSelecionado(Number(e.target.value))}>
